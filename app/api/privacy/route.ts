...
export async function PATCH(
  ...
  try {
    ...
    const contextDb = await db.policiesText.findFirst()
    if (contextDb){
        const newContext = await db.policiesText.update({
            where: {
                id: contextDb.id
        ...
        const newContext = await db.policiesText.create({data: {context: context}})
    }
